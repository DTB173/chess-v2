...
namespace AttackTables {
	...
	consteval std::array<std::array<ui64, 64>, 2> make_pawns_connected_array() {
		std::array<std::array<ui64, 64>, 2> masks{};

		for (int sq = 0; sq < 64; ++sq) {
			ui64 b = 1ULL << sq;
			masks[0][sq] = pawn_attacks<Color::BLACK>(b);
			masks[1][sq] = pawn_attacks<Color::WHITE>(b);
		}
		return masks;
	}

	constexpr inline auto pawns_connected_mask = make_pawns_connected_array();

	constexpr inline ui64 get_connected_mask(Square sq, Color c) {
		return pawns_connected_mask[static_cast<int>(c) - 1][sq];
	}
