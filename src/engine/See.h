...
namespace See {
    ...
    int see(const Position::Position& pos, Move m) {
        ...
        gains[0] = PSQTables::get_material_value(victim_pt).eval(phase);
        ...
        while (from != Square::SQ_NONE) {
            ...
            gains[depth] = PSQTables::get_material_value(attacker.type()).eval(phase) - gains[depth - 1];
            ...
            if (from != Square::SQ_NONE) {
                ...
                if (attacker.type() == PieceType::KING) {
                    ...
                    if (pos.get_attacks_to(to, occupancy, opponent_of(side))) {
                        ...
                        depth++;
                        gains[depth] = PSQTables::get_material_value(PieceType::KING).eval(phase) - gains[depth - 1];
                    }
                    break;
                }
            }
